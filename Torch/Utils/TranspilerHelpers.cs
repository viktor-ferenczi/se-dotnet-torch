...
namespace Torch.Utils
{
    public static class TranspilerHelpers
    {
        ...
        public static void RecordOriginalCode(this List<MsilInstruction> il, LoggingIlGenerator gen, [CallerFilePath] string callerFilePath = "", [CallerMemberName] string callerMemberName = "")
        {
#if DEBUG
            RecordCode(il.Select(msil => msil.ToCodeIns(gen)).ToList(), callerFilePath, callerMemberName, "original");
#endif
        }

        public static void RecordPatchedCode(this List<MsilInstruction> il, LoggingIlGenerator gen, [CallerFilePath] string callerFilePath = "", [CallerMemberName] string callerMemberName = "")
        {
#if DEBUG
            RecordCode(il.Select(msil => msil.ToCodeIns(gen)).ToList(), callerFilePath, callerMemberName, "patched");
#endif
        }
        ...
        private static void RecordCode(List<CodeInstruction> il, string callerFilePath, string callerMemberName, string suffix)
        {
            ...
            if (callerFileName.EndsWith(".cs"))
            {
                callerFileName = callerFileName.Substring(0, callerFileName.Length - 3);
            }
                
            var path = Path.Combine(dir, $"{callerFileName}.{callerMemberName}.{suffix}.il");
